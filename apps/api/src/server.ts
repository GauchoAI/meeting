...
function buildRealtimeInstructions(): string {
  ...
  return [
    ...
    "Keep the spoken conversation responsive even when tools or implementation work are running.",
    "When a tool is accepted in the background, acknowledge briefly and keep listening; do not wait silently for long-running work.",
    "If background work later produces a result, summarize it briefly only when it is useful to the current conversation.",
