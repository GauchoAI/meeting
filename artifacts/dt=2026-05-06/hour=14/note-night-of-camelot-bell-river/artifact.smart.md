...
```mermaid
flowchart TD
  A[Evening: Secret rider arrives] --> B[North Watch assembly]
  B --> C[Alarm: bell rope failure]
  C --> D[Tristan reroutes scouts]
  B --> E[Bedwyr marks hidden ford]
  D --> F[Gawain holds false target at gate]
  E --> G[Raiders diverted from granary route]
  F --> H[Convoy protected]
  G --> H
  H --> I[Dawn: border threat neutralized]
  I --> J[Report to Arthur before sunrise]
  J --> K[No trumpet, no banners, only survival and trust]
```
